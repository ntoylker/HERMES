...
- `alternatives` (merged across parts, post-validation; ungrounded entries and entries duplicating a `top_techniques` `mitre_id` already removed)
...
- An `alternatives` entry whose `mitre_id` already appears in that same part's (post-validation) `top_techniques` is dropped as a duplicate - the prompt instructs the model to keep the two lists disjoint, but this is enforced deterministically rather than trusted.
...
`alternatives` are generated by Gemini from the same prompt context. There is no separate deterministic Python algorithm that computes which techniques become alternatives - but a deterministic step does filter them afterward (see below).

Mechanism:

1. Prompt specifies required JSON schema, including `alternatives`, and states that `alternatives` must be lower-confidence picks that do not repeat a `mitre_id` already in `top_techniques`.
...
4. `validate_generated_links()` drops any `alternatives` entry that is ungrounded or that duplicates a `top_techniques` `mitre_id`, regardless of whether the model followed instruction 1 (section 5.4).
5. Script emits the remaining JSON.

Implication:

- `top_techniques` are retrieval-anchored.
- `alternatives` are model-synthesized, evidence-constrained by provided sources, and deterministically guaranteed to be disjoint from `top_techniques`.
